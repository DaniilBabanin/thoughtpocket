...
- **Production release notes:** the workflow's published body is only boilerplate (auto-changelog + `### Which file?` / `### Build integrity` template). After the release publishes, hand-write a `## What's new` from the real commits since the last production tag (user-facing, honest, humanizer pass), fetch the body with `gh release view vX --json body`, splice the notes above the boilerplate, and apply with `gh release edit vX --notes-file`. Keep the template sections verbatim — their dex witnesses are generated per-build and unrecoverable if overwritten.
